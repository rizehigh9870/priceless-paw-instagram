...
def main() -> int:
    ...
    if not images:
        # リールのみの日（動画だけを置いたフォルダ）は画像が無いのが正常なので、
        # 失敗扱いにせずスキップする。ストーリーズは動画も投稿可能だが、
        # リンクスタンプが手動である以上ここで無理に投稿する必要がないため対象外とする。
        log(f"{folder.name} に画像がありません（リールのみの日と判断）。ストーリーズ投稿をスキップします。")
        return 0
